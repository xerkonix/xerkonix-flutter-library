...
const String _long = '아주 긴 단추 문구가 한 줄에 안 들어가면 부모 너비를 채우지 않고 줄바꿈한다';
...
Widget _bounded({required Widget child}) {
  return MaterialApp(
    ...
    home: Scaffold(
      body: Center(
        child: SizedBox(width: _parentW, child: child),
      ),
    ),
  );
}
...
void main() {
  testWidgets(
    'bounded parent: expanded false shrinks, true fills, long text wraps',
    (WidgetTester tester) async {
      ...
      expect(
        priFalse.height,
        greaterThanOrEqualTo(XkTactileTokens.controlHeight),
      );
      expect(
        secFalse.height,
        greaterThanOrEqualTo(XkTactileTokens.controlHeight),
      ...
      expect(
        priWrap.height,
        greaterThan(XkTactileTokens.controlHeight + 8),
  ...
  testWidgets('field focus/disabled/invalid keep readable labels', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: Column(
            children: <Widget>[
              XkTextInputField(label: '이메일', hintText: 'name@example.com'),
              XkTextInputField(label: '비밀번호', enabled: false),
              XkTextInputField(label: '인증번호', errorText: '코드가 올바르지 않습니다'),
            ],
          ),
        ),
      ),
    ...
    expect(focused.border?.top.color, XkTactileTokens.light.focusRing);
