...
void main() {
  test('glossLow is transparent; sheen/nav/action follow 4.2.1 alphas', () {
    expect(XkColor.glossLow.a, 0);
    expect(XkColor.darkGlossLow.a, 0);
    expect(XkColor.glossSheen.a, closeTo(0.28, 0.01));
    expect(XkColor.glassNavigation.a, closeTo(0.82, 0.01));
    expect(XkColor.glass.a, closeTo(0.75, 0.01));
    expect(XkColor.glassAction.a, closeTo(0.22, 0.01));
  ...
  testWidgets('selected raster is 30% tint over canvas, not a solid aqua bar', (
    ...
    try {
      ...
      // 30% #C0E3EC over #F7F7F7 ≈ (231, 241, 244)
  ...
  testWidgets('debugDisableShadows paints a hard 8px gray stair under glass', (
    ...
    expect(
      band.r,
      lessThan(236),
      reason: 'hard shadow band should sit well below canvas #F7F7F7, got $band',
    );
  });
