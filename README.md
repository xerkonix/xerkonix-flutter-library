...
| 패키지 (pub 이름) | 버전 | 요약 |
|---|---|---|
| [`xerkonix_design_system`](xerkonix-design-system/) | 4.3.0 (패키지, 로컬) | TACTILE 계약 v4.2.0 · 테마·타이포·아이콘·컴포넌트·패턴·모션 위젯 |
...
`xerkonix_design_system` 의 패키지 버전(4.3.0)과 TACTILE 계약 버전(v4.2.0)은 별개다. 패키지 번호는 `xerkonix-design-system/pubspec.yaml` 을, 계약 번호는 `xerkonix-design-system/test/fixtures/tokens.css` 헤더와 `tools/tactile_gate.py` 의 `EXPECT_VERSION` 을 따른다. 주 액션은 검정 역상(`--surface-inverse` + `--ink-inverse`). 아쿠아는 선택·토글·아이콘 강조만.
