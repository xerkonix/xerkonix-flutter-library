...
패키지 버전 4.4.0. 디자인 계약은 TACTILE v4.3.0. 두 번호는 별개다. 신규 공개 API는 `XkGlassRole`과 `XkListRow.surface`. 주 액션은 `--surface-inverse` + `--ink-inverse`. 아쿠아는 작은 선택/토글만.

## 4.3.0 — TACTILE v4.2.0 무채색 베이스 · 검정 CTA

패키지 버전 4.3.0. 디자인 계약은 TACTILE v4.2.0. 두 번호는 별개다. 주 액션은 `--surface-inverse` + `--ink-inverse`. 아쿠아는 작은 선택/토글만.
...
XERKONIX TACTILE 디자인 시스템의 Flutter 구현 패키지. 색·타이포·형태·모션 토큰과 라이트/다크 테마, 아이콘, 컴포넌트, 패턴/모션 위젯을 제공한다. 패키지 버전은 **4.4.0**, 따르는 디자인 계약은 **TACTILE v4.3.0**이다. 두 번호는 별개다. tokens.css 와의 정합은 `test/token_canon_parity_test.dart` 가 매 실행마다 확인하며(형제 리포가 없으면 `test/fixtures/tokens.css` 스냅샷과 대조 — skip 하지 않는다), 그중 **커버리지 그물**은 tokens.css 에 토큰이 새로 생기면 "미러링하거나 이유를 적어라"로 실패한다 — 값 어긋남뿐 아니라 **빠짐**까지 잡는다.
